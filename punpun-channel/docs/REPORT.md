...
## 🆕 v2 追加 (起きた後のフィードバック反映)

起きた直後の感想「地図とか図、アニメーションが欲しい / キャラクターの口パク」を受けて、追加実装:

### 実装したもの

1. **本物の世界/アジア/欧州地図** (`src/video/map_renderer.py`)
   - Cartopy が持つ Natural Earth 110m データを使用
   - 指定国を緑 (#4ade80) で塗る、過去動画と同じ見た目
   - 日本語キーワード対応 (ハンガリー/モンゴル/中国/ローマ帝国 等)

2. **ぷんぷんキャラ改善** (`src/video/character.py`)
   - 少し斜めを向いた角度 (angle=-8°)
   - 口の 3 段階 (closed/half/open)
   - 音声の RMS エンベロープから口パクタイミング算出

3. **アニメーション** (`src/video/animation.py`)
   - Ken Burns (ゆっくりズーム + パン)
   - 吹き出しのポップイン (0.35s イーズアウト)
   - キャラのふわふわ上下 (1.3Hz)
   - シーン頭尾に黒フェード (繋ぎが自然に)
   - 口パクは前後 3 フレームの最大で平滑化 (flicker 防止)

4. **map + 肖像 合成** (`src/video/image_fetcher.py`)
   - `overlay_keyword` 対応: 地図の右上に人物の肖像を重ねる
   - 過去動画のアジア地図 + チンギスハン 肖像のスタイル再現

5. **速度最適化**
   - 背景 resize を 1 回だけに (5x 高速化)
   - 3 並列レンダリング (ProcessPoolExecutor)
   - 失敗キャッシュ (同じ run で失敗した API は再試行しない)

### 短編デモで動作確認済

- `demo/elizabeth_bathory_short_animated.mp4` ← 口パク + Ken Burns + 実地図
