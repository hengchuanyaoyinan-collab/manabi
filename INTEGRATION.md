...
このドキュメントは、`manabi (7).html` と `supabase/` 以下のコードを
...
```
manabi (7).html                                           # クライアント側を Stripe Checkout 経由に変更
...
これらは公開しても安全です。`manabi (7).html` 内で直接定義しています。

| 変数名 | 値 | 使用箇所 |
|---|---|---|
| `SB_URL` | `https://dajngnlvrwesosvgoilh.supabase.co` | ファイル内 `SB_URL` |
| `SB_KEY` | `sb_publishable_TRB_lManD97eMnC74p175g_tm34NK07` | ファイル内 `SB_KEY` |
...
1. ブラウザで `manabi (7).html` を開く (ローカル/ホスティング先どちらでも)
2. 新規ユーザで登録 → ログイン
